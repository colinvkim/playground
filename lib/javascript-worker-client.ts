...
let client: JavaScriptWorkerClient | null = null;
let warmRequested = false;

export function getJavaScriptWorkerClient() {
  if (client) {
    return client;
  }

  const worker = new Worker(new URL("../workers/javascript.worker.ts", import.meta.url), {
    type: "module",
  });
...
export function warmJavaScriptWorker() {
  const nextClient = getJavaScriptWorkerClient();
...
export function resetJavaScriptWorkerClient() {
